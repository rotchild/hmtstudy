/*jslint plusplus: true, vars: true, white: true, nomen: true, node: true, indent: 2 */ 
/*jshint laxcomma: true, laxbreak: true, white: false, node: true */
"use strict";

// gb2312.js provides urlencode in gb2312.
...
module.exports = {
	...
	URLEncode: function (vStr) {
		...
		for (i = 0; i < vStr.length; i++) {
			var ch = vStr.charAt(i);
			if ('~!@#$%^&*()-_+={}[]|\\:;"\'<>,.?/`'.indexOf(ch) !== -1) {
				vGet += '%' + getUCase(ch.charCodeAt(0).toString(16));
			} else if (/[0-9a-zA-Z]/.test(ch)) {
				vGet += ch;
			} else {
				var area;
				for ( area = 0; area < gbData.length; area++) {
					var place = gbData[area].indexOf(ch) + 1;
					if (place) {
						vGet += '%' + getUCase((0xA1 + area).toString(16));
